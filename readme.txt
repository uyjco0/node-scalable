...
-> The current project is building a demo of a scalable and distributed application composed by Node.js-based web and worker nodes with a PostgreSQL master/slave backend. The scalability and distributed properties are built using Apache Zookeeper, Apache Kafka, Confluent Schema Registry, Confluent Rest Proxy, Confluent Bottledwater Plugin/Client, Node.js, Docker, Weave, Supervisor, HAProxy, Logstash, and PostreSQL with Streaming Replication/Logical Decoding:
   - Application behavior:
        ...
        - Each message (i.e. each added file 's line) in Apache Kafka is processed by a 'node-worker' instance (a Node.js-based worker daemon):
             - The 'node-worker' instance is using the Confluent Rest Proxy in order to be able to consume Avro-serialized messages from Apache Kafka:
                  - In turn the Confluent Rest Proxy (and also teh Bottledwater-pg Client) are making use of the Confluent Schema Registry in order to avoid sending with
                    each message the corresponding Avro 's schema (and thus saving bandwith/latency)
             - For each message the 'node-worker' instance in turn is making a query to the PostgreSQL backend:
                  - So depending of the rate of CSV files being loaded to the database, and the number of lines that each CSV file has, it is possible to have thousand
                    of concurrent connections to the database
...
-> Where to start?:
      - node-scalable/07-docs/readme.txt
